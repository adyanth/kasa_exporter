...
async def scrape_ip(
    ...
    try:
        ...
        today_counter.labels(alias=dev.alias, model=dev.model).set(
            energy.consumption_today
        )
        this_month_counter.labels(alias=dev.alias, model=dev.model).set(
...
async def main():
    ...
    p_gauge = Gauge(
        "plug_power_watts", "Current power consumed in Watts", ["alias", "model"]
    )
    v_gauge = Gauge(
        "plug_voltage_volts", "Current plug voltage in Volts", ["alias", "model"]
    )
    i_gauge = Gauge(
        "plug_current_amps", "Current current flow in Amps ", ["alias", "model"]
    )
    today_counter = Gauge(
        "plug_consumption_today_kwh",
        "Total energy consumption today in KWh",
        ["alias", "model"],
    )
    this_month_counter = Gauge(
        "plug_consumption_this_month_kwh",
        "Total energy consumtion this month in KWh",
        ["alias", "model"],
    )
